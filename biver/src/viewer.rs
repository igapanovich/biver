...
use egui::{
    ColorImage, Image, Key, Rect, TextureHandle, TextureOptions, Ui, ViewportBuilder,
...
pub fn show_preview(image_path: &Path) -> Result<()> {
    let image = egui_image_from_file(image_path)?;

    eframe::run_native(
        "",
        egui_options(),
        Box::new(|cc| Ok(Box::new(PreviewApp::new(cc, image)))),
    )?;
...
fn egui_image_from_file(path: &Path) -> Result<ColorImage> {
...
impl eframe::App for PreviewApp {
    fn ui(&mut self, ui: &mut Ui, _frame: &mut Frame) {
        let (q_pressed, f_pressed) = ui.input(|i| (i.key_pressed(Key::Q), i.key_pressed(Key::F)));
        ...
        if f_pressed {
            ...
            ui.send_viewport_cmd(ViewportCommand::Title(
                if self.flipped { "(flipped) " } else { "" }.to_string(),
            ));
        }
        ...
        egui::CentralPanel::default().show(ui, |ui| {
            ...
            ui.add(
                Image::new(&self.image_texture)
                    .fit_to_exact_size(ui_size)
                    .uv(uv_rect(self.flipped)),
            );
        });
    }
}
...
impl<'a> ComparerApp<'a> {
    fn new(
        ...
        Self {
            image1_texture: cc
                .egui_ctx
                .load_texture("image1", image1, TextureOptions::default()),
            image2_texture: cc
                .egui_ctx
                .load_texture("image2", image2, TextureOptions::default()),
            description1,
            description2,
...
impl<'a> eframe::App for ComparerApp<'a> {
    fn ui(&mut self, ui: &mut Ui, _frame: &mut Frame) {
        let (q_pressed, k_pressed, j_pressed, space_pressed, f_pressed) = ui.input(|i| {
            (
                i.key_pressed(Key::Q),
                i.key_pressed(Key::K),
                i.key_pressed(Key::J),
        ...
        if title_should_be_updated {
            ...
            ui.send_viewport_cmd(ViewportCommand::Title(description));
        }
